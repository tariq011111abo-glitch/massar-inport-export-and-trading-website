...
export async function sendContactNotification(formData: {
  ...
  try {
    ...
    // جلب رابط شعار الشركة ديناميكياً من قاعدة البيانات لتضمينه في الهيدر، أو استخدام الرابط الاحتياطي
    ...
    // 1️⃣ الإيميل الأول: يرسل تفاصيل الاستفسار الواردة مباشرة إلى إيميل شركتك الرسمي
    // بعد تفعيل الـ DMARC في GoDaddy، سيقبل سيرفر الشركة استقبال الاسم الطويل المحمي دون ارتداد أحمر
    try {
    ...
    try {
      ...
      const autoReplyHtml = `
        <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: 0 auto; background-color: #fcfbf7; border-radius: 24px; border: 1px solid #d4af37; overflow: hidden; box-shadow: 0 4px 20px rgba(0,0,0,0.05);">
          ...
          <!-- هيدر احترافي يحتوي على شعار موقع مسار واسم الشركة المحدث -->
          ...
          <!-- محتوى رسالة الرد التلقائي المخصصة والمنسقة حسب لغة العميل -->
          <div style="padding: 40px 30px; color: #1c2d24; line-height: 1.7; direction: ${isArabic ? 'rtl' : 'ltr'}; text-align: ${isArabic ? 'right' : 'left'};">
            ...
            ` : `
              ...
              <!-- دمج نص رسالتك المخصصة والكاملة بدقة مع روابط الواتساب المباشرة القابلة للضغط -->
              <div style="background-color: #f3f1e9; border-left: 4px solid #d4af37; padding: 20px; border-radius: 12px; margin: 25px 0;">
            ...
            <!-- التوقيع النهائي باسم شركتك الرسمي المحدث -->
      ...
      console.log("Customer auto-reply email sent successfully.");
    } catch (userErr) {
